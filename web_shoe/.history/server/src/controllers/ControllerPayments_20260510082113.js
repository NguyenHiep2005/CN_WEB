    ...
    async updateProductQuantity(products) {
        try {
            for (const product of products) {
                ...
                if (productInDb && productInDb.sizes && productInDb.sizes.length > 0) {
                    // Tìm size phù hợp - so sánh cả dạng string và number
                    const sizeIndex = productInDb.sizes.findIndex(s => 
                        s.size == product.size || s.size === product.size.toString()
                    );
                    
                    if (sizeIndex !== -1) {
                        // Giảm số lượng
                        productInDb.sizes[sizeIndex].quantity -= product.quantity;
                        
                        // Đảm bảo số lượng không âm
                        if (productInDb.sizes[sizeIndex].quantity < 0) {
                            productInDb.sizes[sizeIndex].quantity = 0;
                        }
                        
                        // Lưu lại
                        await productInDb.save();
                        console.log(`Updated product: ${product.nameProduct}, size: ${product.size}, remaining: ${productInDb.sizes[sizeIndex].quantity}`);
                    } else {
                        console.warn(`Size not found for product ${product.nameProduct}, size: ${product.size}`);
                    }
                } else {
