...
class ControllerPayments {
    ...
    async checkData(req, res) {
        try {
            ...
            // Check if orderInfo is a pending order ID (buy now) or user email
            let email = '';
            let pendingOrder = null;
            
            try {
                // Try to treat it as a pending order ID
                if (orderInfo.match(/^[0-9a-f]{24}$/)) {
                    pendingOrder = await ModelBuyNowPending.findById(orderInfo);
                    if (pendingOrder) {
                        email = pendingOrder.user;
                    }
                }
            } catch (err) {
                // Not a valid ObjectId, treat as email
            }

            // If not found as pending order, treat as email
            if (!pendingOrder) {
                email = orderInfo;
            }
            ...
            // If buy now order exists, use its data
            if (pendingOrder) {
                products = pendingOrder.products;
                phone = pendingOrder.phone;
                username = pendingOrder.name;
                finalAddress = pendingOrder.address;
                ...
                // Nếu không có cart và không có pending order, just use email info
                username = dataUser ? dataUser.fullname : '';
                phone = '';
            }
            ...
            // Delete cart and pending order if they exist
            if (cart) {
