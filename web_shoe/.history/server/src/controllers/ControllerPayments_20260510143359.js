...
class ControllerPayments {
    ...
    async getRevenueStatistics(req, res) {
        try {
            ...
            // Process each payment - COUNT ALL, NOT JUST COMPLETED
            payments.forEach(payment => {
                // Count ALL users (regardless of payment status)
                if (payment.user) {
                    totalUsers.add(payment.user);
                }

                // Only count revenue from completed/paid orders (trangthai === true)
                // But count all orders for total orders
                ...
                if (paymentDate >= weekStart) {
                    ...
                    if (dayIndex >= 0 && dayIndex < 7) {
                        // Only add to stats if payment is paid (trangthai === true)
                        if (payment.trangthai === true) {
                            // Count quantity
                            ...
                            // Add revenue
                            weekStats[dayIndex].revenue += payment.sumprice || 0;
                            totalRevenue += payment.sumprice || 0;
                            totalOrders++;
                        }
                    }
                } else if (paymentDate < weekStart && payment.trangthai === true) {
                    // Count total orders from all time (not just this week)
                    if (payment.trangthai === true) {
                        totalOrders++;
                    }
                }
            ...
            const uniqueUserCount = totalUsers.size;

            console.log(`Total orders (paid): ${totalOrders}, Total users: ${uniqueUserCount}, Total revenue: ${totalRevenue}`);
