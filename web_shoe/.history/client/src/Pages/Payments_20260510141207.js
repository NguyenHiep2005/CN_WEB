...
function Payments() {
    ...
    useEffect(() => {
        // Only update cart info if user is authenticated and all fields are filled
        ...
        const fetchData = async () => {
            try {
                ...
                console.log('Cart info updated:', res);
            } catch (error) {
                // Silently fail if not authenticated - this is OK
                // User doesn't need to be logged in to checkout
                console.log('Could not update cart info (auth not required):', error.message);
            }
        };

        // Debounce: only call after 500ms of no changes
    ...
    useEffect(() => {
        ...
        if (buyNowProduct) {
            ...
            request.get('/api/cart')
                ...
                .catch((error) => {
                    console.log('Lỗi khi tải giỏ hàng:', error.message);
                    // Nếu không có giỏ hàng, đặt giỏ hàng rỗng
                    setDataCart([]);
                    setDataProducts([]);
                });
        }
