# 👟 Web Shoe - Nền Tảng Bán Giày Thể Thao Online

## 📋 Mục Lục

- [Giới Thiệu](#giới-thiệu)
- [Tính Năng](#tính-năng)
- [Yêu Cầu Hệ Thống](#yêu-cầu-hệ-thống)
- [Cài Đặt & Chạy Ứng Dụng](#cài-đặt--chạy-ứng-dụng)
- [Cấu Trúc Dự Án](#cấu-trúc-dự-án)
- [Công Nghệ Sử Dụng](#công-nghệ-sử-dụng)
- [Cấu Hình Môi Trường](#cấu-hình-môi-trường)
- [API Endpoints](#api-endpoints)
- [Docker](#docker)

---

## 🎯 Giới Thiệu

**Web Shoe** là một nền tảng thương mại điện tử hiện đại cho bán lẻ giày thể thao. Ứng dụng được xây dựng theo mô hình **Client-Server** với Frontend và Backend tách rời, cho phép quản lý sản phẩm, người dùng, đơn hàng và thanh toán một cách hiệu quả.

---

## ✨ Tính Năng

### 👤 Người Dùng
- ✅ Đăng ký, đăng nhập tài khoản
- ✅ Quên mật khẩu & đặt lại mật khẩu
- ✅ Xem và chỉnh sửa thông tin cá nhân
- ✅ Danh sách yêu thích (Wishlist)

### 🛒 Mua Sắm
- ✅ Duyệt sản phẩm theo danh mục
- ✅ Tìm kiếm và lọc sản phẩm
- ✅ Xem chi tiết sản phẩm
- ✅ Thêm vào giỏ hàng
- ✅ Quản lý giỏ hàng (cập nhật, xóa)

### 💳 Thanh Toán
- ✅ Thanh toán qua VNPay (Internet Banking)
- ✅ Xem lịch sử đơn hàng
- ✅ Theo dõi trạng thái đơn hàng
- ✅ Xác nhận thanh toán thành công

### 📊 Admin Dashboard
- ✅ Quản lý sản phẩm (Thêm, sửa, xóa)
- ✅ Quản lý danh mục
- ✅ Quản lý người dùng
- ✅ Quản lý đơn hàng
- ✅ Thống kê doanh thu và lượng bán
- ✅ Biểu đồ phân tích (Chart.js)

### 💬 Tính Năng Khác
- ✅ Chatbot hỗ trợ khách hàng (Google Generative AI)
- ✅ Gửi email thông báo
- ✅ Real-time notification (Socket.io)

---

## 🔧 Yêu Cầu Hệ Thống

### Bắt Buộc
- **Node.js** v14 trở lên
- **npm** v6+ hoặc **yarn**
- **MongoDB** (Local hoặc Cloud: MongoDB Atlas)

### Tùy Chọn
- **Docker** (để chạy ứng dụng trong container)
- **Git** (để clone repository)

---

## 💻 Cài Đặt & Chạy Ứng Dụng

### 1️⃣ Clone Repository

```bash
git clone <repository-url>
cd web_shoe
```

### 2️⃣ Cài Đặt Backend

```bash
cd server
npm install
```

#### Tạo file `.env` trong thư mục `server/`:

```env
# Server Configuration
PORT=5000
NODE_ENV=development

# MongoDB Connection
MONGODB_URI=mongodb+srv://username:password@cluster.mongodb.net/web_shoe
# Hoặc local: mongodb://localhost:27017/web_shoe

# JWT Secret
JWT_SECRET=your_jwt_secret_key_here
JWT_EXPIRE=7d

# CORS
REACT_APP_URL=http://localhost:3001

# Email Configuration (Nodemailer)
EMAIL_USER=your_email@gmail.com
EMAIL_PASSWORD=your_app_password

# VNPay Configuration
VNP_TMNCODE=your_vnpay_merchant_code
VNP_HASHSECRET=your_vnpay_hash_secret
VNP_RETURNURL=http://localhost:3001/payment-success
VNP_NOTIFYURL=http://localhost:5000/api/payment/vnpay-ipn

# Google Generative AI (Chatbot)
GOOGLE_API_KEY=your_google_generative_ai_key
```

#### Chạy Server:

```bash
# Development (với hot-reload)
npm start

# Hoặc chạy trực tiếp
npm run start
```

Server sẽ chạy tại `http://localhost:5000`

---

### 3️⃣ Cài Đặt Frontend

```bash
cd client
npm install
```

#### Tạo file `.env` trong thư mục `client/`:

```env
# API Configuration
REACT_APP_API_URL=http://localhost:5000/api
REACT_APP_SOCKET_URL=http://localhost:5000

# Environment
NODE_ENV=development
```

#### Chạy Client:

```bash
npm start
```

Client sẽ mở tự động tại `http://localhost:3001`

---

## 📁 Cấu Trúc Dự Án

```
web_shoe/
│
├── client/                          # Frontend (React)
│   ├── public/
│   │   ├── index.html
│   │   └── manifest.json
│   │
│   ├── src/
│   │   ├── Components/              # React Components
│   │   │   ├── Header.js
│   │   │   ├── Navbar.js
│   │   │   ├── Footer.js
│   │   │   ├── ManageProducts.js    # Quản lý sản phẩm (Admin)
│   │   │   ├── ManageOrder.js       # Quản lý đơn hàng (Admin)
│   │   │   ├── ManagerUser.js       # Quản lý người dùng (Admin)
│   │   │   └── ...
│   │   │
│   │   ├── Pages/                   # Trang chính
│   │   │   ├── Home.js
│   │   │   ├── Login.js
│   │   │   ├── RegisterUser.js
│   │   │   ├── Cart.js
│   │   │   ├── DetailProducts.js
│   │   │   ├── Admin.js
│   │   │   ├── PaymentSuccess.js
│   │   │   └── ...
│   │   │
│   │   ├── Config/
│   │   │   └── api.js               # Cấu hình API endpoints
│   │   │
│   │   ├── store/                   # State Management (Context API)
│   │   │   ├── Context.js
│   │   │   └── Provider.js
│   │   │
│   │   ├── hooks/                   # Custom Hooks
│   │   │   ├── useDebounce.js
│   │   │   └── useStore.js
│   │   │
│   │   ├── Styles/                  # SCSS/CSS Modules
│   │   │   ├── Admin.module.scss
│   │   │   ├── Cart.module.scss
│   │   │   └── ...
│   │   │
│   │   ├── utils/                   # Utilities
│   │   │   ├── Modal/
│   │   │   ├── HandleCart/
│   │   │   ├── Chatbot/
│   │   │   └── formatVietnamTime.js
│   │   │
│   │   ├── App.js
│   │   └── index.js
│   │
│   ├── Dockerfile
│   └── package.json
│
├── server/                          # Backend (Node.js/Express)
│   ├── src/
│   │   ├── Config/
│   │   │   └── db.js                # MongoDB Connection
│   │   │
│   │   ├── controllers/             # Logic kinh doanh
│   │   │   ├── productController.js
│   │   │   ├── orderController.js
│   │   │   ├── userController.js
│   │   │   └── ...
│   │   │
│   │   ├── models/                  # MongoDB Models/Schemas
│   │   │   ├── User.js
│   │   │   ├── Product.js
│   │   │   ├── Order.js
│   │   │   └── ...
│   │   │
│   │   ├── routes/                  # API Routes
│   │   │   ├── userRoutes.js
│   │   │   ├── productRoutes.js
│   │   │   ├── orderRoutes.js
│   │   │   └── ...
│   │   │
│   │   ├── jwt/                     # JWT Authentication
│   │   │   └── config.js
│   │   │
│   │   ├── SendMail/                # Email Service
│   │   │   └── ...
│   │   │
│   │   ├── utils/                   # Utilities
│   │   │   └── ...
│   │   │
│   │   ├── uploads/                 # Upload files
│   │   │   └── ...
│   │   │
│   │   └── server.js                # Entry point
│   │
│   ├── public/
│   │   └── images/
│   │       └── products/
│   │
│   ├── Dockerfile
│   └── package.json
│
├── BAO_CAO_CLIENT_SERVER.md         # Báo cáo kiến trúc
└── README.md
```

---

## 🛠️ Công Nghệ Sử Dụng

### Frontend
| Công Nghệ | Phiên Bản | Mục Đích |
|-----------|---------|---------|
| React | ^18.3.1 | UI Framework |
| React Router | ^6.23.1 | Navigation |
| React Redux | ^9.1.2 | State Management |
| Axios | ^1.7.2 | HTTP Client |
| Bootstrap | ^5.3.3 | UI Components |
| SCSS/SASS | ^1.77.4 | Styling |
| Chart.js | ^4.4.4 | Biểu đồ thống kê |
| Socket.io-client | ^4.7.5 | Real-time |

### Backend
| Công Nghệ | Phiên Bản | Mục Đích |
|-----------|---------|---------|
| Express.js | ^4.19.2 | Web Framework |
| MongoDB | 8.3.2 | Database |
| Mongoose | ^8.3.2 | ODM |
| JWT | ^9.0.2 | Authentication |
| Bcrypt | ^5.1.1 | Password Hashing |
| Nodemailer | ^6.9.13 | Email Service |
| Multer | ^1.4.5 | File Upload |
| Socket.io | ^4.7.5 | Real-time |
| VNPay SDK | ^2.1.1 | Payment Gateway |
| Google Generative AI | ^0.24.0 | Chatbot |

---

## ⚙️ Cấu Hình Môi Trường

### Backend `.env`

```env
# Server
PORT=5000
NODE_ENV=development

# Database
MONGODB_URI=mongodb+srv://user:password@cluster.mongodb.net/web_shoe

# JWT
JWT_SECRET=your_secret_key_here
JWT_EXPIRE=7d

# Client URL
REACT_APP_URL=http://localhost:3001

# Email
EMAIL_USER=your_email@gmail.com
EMAIL_PASSWORD=your_app_password

# Payment
VNP_TMNCODE=merchant_code
VNP_HASHSECRET=hash_secret
VNP_RETURNURL=http://localhost:3001/payment-success
VNP_NOTIFYURL=http://localhost:5000/api/payment/vnpay-ipn

# AI
GOOGLE_API_KEY=your_api_key
```

### Frontend `.env`

```env
REACT_APP_API_URL=http://localhost:5000/api
REACT_APP_SOCKET_URL=http://localhost:5000
NODE_ENV=development
```

---

## 🔌 API Endpoints

### Authentication
```
POST   /api/auth/register           - Đăng ký
POST   /api/auth/login              - Đăng nhập
POST   /api/auth/logout             - Đăng xuất
POST   /api/auth/refresh-token      - Làm mới token
```

### Products
```
GET    /api/products                - Danh sách sản phẩm
GET    /api/products/:id            - Chi tiết sản phẩm
POST   /api/products                - Tạo sản phẩm (Admin)
PUT    /api/products/:id            - Cập nhật sản phẩm (Admin)
DELETE /api/products/:id            - Xóa sản phẩm (Admin)
```

### Orders
```
GET    /api/orders                  - Danh sách đơn hàng
GET    /api/orders/:id              - Chi tiết đơn hàng
POST   /api/orders                  - Tạo đơn hàng
PUT    /api/orders/:id              - Cập nhật đơn hàng
```

### Users
```
GET    /api/users                   - Danh sách người dùng (Admin)
GET    /api/users/:id               - Chi tiết người dùng
PUT    /api/users/:id               - Cập nhật người dùng
DELETE /api/users/:id               - Xóa người dùng (Admin)
```

### Payment
```
POST   /api/payment/create-vnpay    - Tạo link thanh toán VNPay
GET    /api/payment/vnpay-return    - Xử lý callback VNPay
```

---

## 🐳 Docker

### Chạy với Docker Compose

1. **Tạo file `docker-compose.yml` ở root directory:**

```yaml
version: '3.8'

services:
  # MongoDB
  mongodb:
    image: mongo:latest
    container_name: web_shoe_db
    ports:
      - "27017:27017"
    environment:
      MONGO_INITDB_ROOT_USERNAME: admin
      MONGO_INITDB_ROOT_PASSWORD: password
    volumes:
      - mongo_data:/data/db

  # Backend Server
  server:
    build: ./server
    container_name: web_shoe_server
    ports:
      - "5000:5000"
    environment:
      MONGODB_URI: mongodb://admin:password@mongodb:27017/web_shoe?authSource=admin
      PORT: 5000
      NODE_ENV: production
      JWT_SECRET: your_secret_key
      REACT_APP_URL: http://localhost:3001
    depends_on:
      - mongodb
    networks:
      - web_shoe_network

  # Frontend Client
  client:
    build: ./client
    container_name: web_shoe_client
    ports:
      - "3001:3000"
    environment:
      REACT_APP_API_URL: http://localhost:5000/api
      REACT_APP_SOCKET_URL: http://localhost:5000
    depends_on:
      - server
    networks:
      - web_shoe_network

volumes:
  mongo_data:

networks:
  web_shoe_network:
    driver: bridge
```

2. **Chạy ứng dụng:**

```bash
docker-compose up -d
```

3. **Dừng ứng dụng:**

```bash
docker-compose down
```

---

## 🚀 Hướng Dẫn Chạy Nhanh

### Development Mode (Local)

```bash
# Terminal 1 - Chạy Backend
cd server
npm install
npm start

# Terminal 2 - Chạy Frontend
cd client
npm install
npm start
```

**Kết quả:**
- Frontend: http://localhost:3001
- Backend API: http://localhost:5000

### Production Mode (Docker)

```bash
# Build và chạy Docker containers
docker-compose up --build

# Hoặc chạy background
docker-compose up -d
```

---

## 📝 Script Hữu Ích

### Backend
```bash
# Cài đặt dependencies
npm install

# Chạy server (development)
npm start

# Seed database
node seed.js

# Tạo placeholder images
node create-placeholder-images.js
```

### Frontend
```bash
# Cài đặt dependencies
npm install

# Chạy development server
npm start

# Build cho production
npm run build

# Chạy tests
npm test
```

---

## 🔐 Bảo Mật

- ✅ JWT Token Authentication
- ✅ Password Hashing (Bcrypt)
- ✅ CORS Configuration
- ✅ Environment Variables
- ✅ Cookie Parser
- ✅ Body Parser Validation

---

## 📱 Tính Năng Responsive

- ✅ Mobile-first Design
- ✅ Bootstrap Grid System
- ✅ Responsive Images
- ✅ Touch-friendly UI

---

## 🐛 Troubleshooting

### MongoDB Connection Error
```
Lỗi: Cannot connect to MongoDB
Giải pháp:
- Kiểm tra MONGODB_URI trong .env
- Đảm bảo MongoDB đang chạy
- Kiểm tra credentials
```

### Port Already in Use
```
Lỗi: Port 3001 / 5000 đang được sử dụng
Giải pháp:
- Thay đổi PORT trong .env
- Hoặc kill process đang chiếm port
```

### CORS Error
```
Lỗi: CORS policy error
Giải pháp:
- Kiểm tra REACT_APP_URL trong .env backend
- Đảm bảo cùng domain/port
```

---

## 👨‍💻 Đóng Góp

Chúng tôi hoan nghênh các đóng góp! Vui lòng:
1. Fork repository
2. Tạo branch mới (`git checkout -b feature/AmazingFeature`)
3. Commit changes (`git commit -m 'Add some AmazingFeature'`)
4. Push to branch (`git push origin feature/AmazingFeature`)
5. Open a Pull Request

---

## 📄 License

Dự án này được cấp phép dưới MIT License - xem file LICENSE để biết chi tiết.

---

## 📞 Liên Hệ & Support

- 📧 Email: support@webshoe.com
- 💬 Facebook: https://facebook.com/webshoe
- 🌐 Website: https://webshoe.com
- 📱 Hotline: 1900-1000

---

## ✅ Changelog

### v1.0.0 (2026-05-27)
- ✨ Khởi chạy dự án
- ✅ Tính năng authentication
- ✅ Quản lý sản phẩm & đơn hàng
- ✅ Thanh toán VNPay
- ✅ Admin Dashboard
- ✅ Chatbot AI

---

**Happy Coding! 🚀**
