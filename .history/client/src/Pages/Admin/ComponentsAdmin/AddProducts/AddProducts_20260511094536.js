...
function AddProducts({ setCheckOpenAddProduct, onProductAdded }) {
    ...
    const handleAddProduct = async () => {
        // Validate
        if (!nameProduct || !priceProduct || !checkType || !categoryId || sizes.some(s => !s.size || s.quantity < 0)) {
            toast.error('Vui lòng điền đầy đủ thông tin sản phẩm. Tất cả size phải có tên và số lượng >= 0. Vui lòng chọn danh mục.');
            return;
        }
