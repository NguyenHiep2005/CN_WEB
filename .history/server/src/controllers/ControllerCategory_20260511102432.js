...
class ControllerCategory {
    ...
    async ApplyDiscount(req, res, next) {
        try {
            ...
            console.log('ApplyDiscount called:', { id, discountPercentage });
            ...
            console.log(`Found ${productsToDiscount.length} products for category ${category.name}`);
            ...
            const updatePromises = productsToDiscount.map((product) => {
                ...
                console.log(`Updating product ${product.name}: ${basePrice} -> ${newPrice}`);
            ...
            console.log(`Successfully applied discount to ${updatePromises.length} products`);
    ...
    async RemoveDiscount(req, res, next) {
        try {
            ...
            console.log('RemoveDiscount called:', { id });
            ...
            console.log(`Found ${productsToRestore.length} discounted products to restore`);

            // Restore original prices
            const updatePromises = productsToRestore.map((product) => {
                console.log(`Restoring product ${product.name}: ${product.price} -> ${product.originalPrice}`);
            ...
            console.log(`Successfully restored ${updatePromises.length} products`);
            ...
            return res.status(500).json({ message: 'Error removing discount', error: error.message });
        }
    }
