...
class ControllerPayments {
    ...
    async getRevenueStatistics(req, res) {
        try {
            ...
            // First pass: Count total users and total orders (all time)
            payments.forEach(payment => {
                ...
                // Count total COMPLETED orders (trangthai === true)
                if (payment.trangthai === true) {
                    totalOrders++;
                }
            });

            // Second pass: Calculate weekly stats for completed orders only
            payments.forEach(payment => {
